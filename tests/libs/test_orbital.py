...
class Testorbital(unittest.TestCase):
    ...
    def test_orbital_inclination_args_none(self):
        ...
        self.assertRaises(TypeError, orbital_inclination, None, 0, 0, 0)
        self.assertRaises(TypeError, orbital_inclination, self._r1_sol_earth, None, 0, 0)
        self.assertRaises(TypeError, orbital_inclination, self._r1_sol_earth, 0, None, 0)
        self.assertRaises(TypeError, orbital_inclination, self._r1_sol_earth, 0, 0, None)
    ...
    def test_orbital_inclination_on_edge_on_orbit(self):
        ...
        for e, esinw, eclipse in [(0, 0, EclipseType.PRIMARY), (0, 0, EclipseType.SECONDARY)]:
            inc = orbital_inclination(**kwargs, e=e, esinw=esinw, eclipse=eclipse)
            self.assertEqual(inc.value, 90)

    def test_orbital_inclination_known_impact_parameters(self):
        """ Tests orbital_inclination(known r1, b, e and esinw) -> known inc values from fitting """
        for kwargs in [self._cw_eri.copy(), self._v570_per.copy(), self._zeta_phe.copy()]:
            (bp, bs) = impact_parameter(**kwargs, eclipse=EclipseType.BOTH)
            exp_inc = kwargs.pop("inc").to(u.deg).value
            kwargs["esinw"] = kwargs["e"] * np.sin(kwargs.pop("omega").to(u.rad).value)

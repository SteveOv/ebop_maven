...
class Testlightcurve(unittest.TestCase):
    ...
    #
    def test_find_lightcurve_segments_cw_eri(self):
        """ Tests find_lightcurve_segments(CW Eri sector 31, th=0.5 d) -> 2 segments """
        lc = th.load_lightcurve("CW Eri")
        segmment_ixs = list(lightcurve.find_lightcurve_segments(lc, TimeDelta(0.5 * u.d)))
        self.assertEqual(len(segmment_ixs), 2)
        for seg_ixs, exp_times in zip(segmment_ixs, [(2144.5, 2156.7),
                                                     (2158.9, 2169.9)]):
            self.assertAlmostEqual(lc[seg_ixs[0]]["time"].value, exp_times[0], 1)
            self.assertAlmostEqual(lc[seg_ixs[1]]["time"].value, exp_times[1], 1)
    ...
    def test_find_lightcurve_segments_v889_aql(self):
        ...
        segmment_ixs = list(lightcurve.find_lightcurve_segments(lc, TimeDelta(0.5 * u.d)))
        self.assertEqual(len(segmment_ixs), 2)
        for seg_ixs, exp_times in zip(segmment_ixs, [(2390.7, 2404.4),
                                                     (2405.3, 2418.9)]):
            self.assertAlmostEqual(lc[seg_ixs[0]]["time"].value, exp_times[0], 1)
            self.assertAlmostEqual(lc[seg_ixs[1]]["time"].value, exp_times[1], 1)
    ...
    def test_find_lightcurve_segments_v889_aql_high_threshold(self):
        ...
        lc = th.load_lightcurve("V889 Aql")
        segmment_ixs = list(lightcurve.find_lightcurve_segments(lc, TimeDelta(1.0 * u.d)))
        self.assertEqual(len(segmment_ixs), 1)
        for seg_ixs, exp_times in zip(segmment_ixs, [(2390.7, 2418.9)]):
            self.assertAlmostEqual(lc[seg_ixs[0]]["time"].value, exp_times[0], 1)
            self.assertAlmostEqual(lc[seg_ixs[1]]["time"].value, exp_times[1], 1)
